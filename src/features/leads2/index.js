...
function Leads2() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:8080/product/addSubcategory", { subcategory });
            setSubcategory("");
            dispatch(showNotification({ message: "Subcategory added successfully!", status: 1 }));
        } catch (error) {
            console.error("Error adding subcategory:", error);
            dispatch(showNotification({ message: "Failed to add subcategory.", status: 2 }));
        }
    };
        ...
        <TitleCard title="Current Leads" topMargin="mt-2" TopSideButtons={<TopSideButtons />}>
            <div className="flex justify-center items-center min-h-screen bg-gray-100 dark:bg-gray-900">
                <div className="w-full max-w-3xl bg-white text-black dark:bg-gray-800 dark:text-white p-8 rounded-lg shadow-lg">
                    <h2 className="text-2xl font-bold text-center mb-6">Subcategory Wizard</h2>
